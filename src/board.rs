...
fn select_square(
    ...
    // Only run if the left mouse button is clicked
    if !mouse_button_inputs.just_pressed(MouseButton::Left) {
        return;
    }

    // Get the square under the cursoe and set it as selected
    selected_square.entity = if let Some((entity, _insersection)) = pick_state.top(Group::default())
    {
        Some(*entity)
    } else {
        None
